...
#        testNum = len(testData)
        ...
        outfile = open("poly3.txt","w")


        for g in [5, 10, 50 , 100]:
            for c in [1,10,50,100,500,1000]:
                clf = SVC(kernel = 'poly', degree = 3, gamma = g, coef0 = 1)
                clf.fit(trainX, trainy)
                trainscore = str(clf.score(trainX, trainy))
                testscore = str(clf.score(testX, testy))
                print("C = " + str(c) + ", kernel = poly , gamma = " + str(g) + ", degree = 3 , train score :  " + trainscore + ", test score: " + testscore)
                outfile.write("C = " + str(c) + ", kernel = poly , gamma = " + str(g) + ", degree = 3 , train score :  " + trainscore + ", test score: " + testscore + '\n')
