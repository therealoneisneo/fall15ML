...
#        testNum = len(testData)
        ...
        for g in [0.0, 0.1, 0.5, 1.0]:
            for c in [1,10,50,100,1000]:
                clf = SVC(kernel = 'poly', degree = 1, gamma = g, coef0 = 1)
                clf.fit(trainX, trainy)
                trainscore = str(clf.score(trainX, trainy))
                testscore = str(clf.score(testX, testy))
                print("C = " + str(c) + ", kernel = poly , gamma = " + str(g) + ", degree = 1 , train score :  " + trainscore + ", test score: " + testscore)
                outfile.write("C = " + str(c) + ", kernel = poly , gamma = " + str(g) + ", degree = 1 , train score :  " + trainscore + ", test score: " + testscore + '\n')
