...
if __name__ == "__main__":
    ...
    Cords = SphereDataGen( 100, 20, 130, 20)
...
#     GrayDisM = t.ReadDisMatrix("distanceSy.txt")
    DisM = t.EDistanceOfVecs(Cords)
    oset = iso.ObjectSet()

    oset.addObjects(DisM)
 
    oset.findNeighborhoods(20)
    
    oset.buildGeodesicCache()
    
    GeoDisM = oset.getObjectDeltas()
    
    CMass1 = CenterOfMass(Cords)
    
    CMass = Topcords(GeoDisM, Cords)
     
    RctCords = Rectify(Cords, CMass)
